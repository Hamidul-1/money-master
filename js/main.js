...
document.getElementById('calculate-btn').addEventListener('click', function (){
    ...
    const foodInput = getInputValue('food-expenses');
    const rentInput = getInputValue('rent-expenses');
    const clothesInput = getInputValue('clothes-expenses');
...
document.getElementById('save-button').addEventListener('click', function () {
    ...
    const incomeInput = getInputValue('income-value');
    const savingInputValue = getInputValue('save-input');
...
/* const saveButton = document.getElementById('save-button').addEventListener('click',function(){
    ...
    const amountSave = saveButton.value;
    const balance = document.getElementById('total-balance').innerText;

    // document.getElementById('total-balance').innerText = balance;
    let Saveing = parseFloat(incomeAmount) / 100;
    let totalSavings = Saveing * amountSave;


    let finalBalance = parseFloat(balance) - parseFloat(totalSavings);
    // balance
    
    document.getElementById('saving-amount').innerText = totalSavings;
    document.getElementById('remaining-balance').innerText = finalBalance;
}) */
